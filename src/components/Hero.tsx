...
export default function Hero() {
  ...
  return (
    <section
      id="hem"
      className="relative overflow-hidden bg-gradient-to-br from-[#4A2D42] via-[#7B4B6F] to-[#7B4B6F]/80 text-white"
    >
      {heroImage && (
        <>
          <img
            ...
            fetchPriority="high"
          />
          <div className="absolute inset-0 bg-[#4A2D42]/70" aria-hidden />
        </>
      )}
      {!heroImage && (
        <div className="absolute inset-0 opacity-20" aria-hidden>
          <div className="absolute -right-20 top-10 h-72 w-72 rounded-full bg-[#F4C2C2] blur-3xl" />
      ...
      <div className="relative mx-auto grid max-w-6xl gap-10 px-4 py-20 sm:px-6 lg:grid-cols-2 lg:items-center lg:py-28">
        <div>
          <p className="mb-4 inline-flex items-center gap-2 rounded-full bg-[#F4C2C2]/20 px-4 py-1.5 text-sm font-semibold text-[#F4C2C2] ring-1 ring-[#F4C2C2]/30">
            <span aria-hidden>★</span>
            {business.rating}/5 · {business.reviewCount}+ omdömen
          </p>
          <h1 className="text-4xl font-bold leading-tight tracking-tight sm:text-5xl lg:text-6xl">
            Fransar, naglar & brows
            <span className="block text-[#F4C2C2]">under ett tak i Gottsunda</span>
          </h1>
          ...
          <div className="mt-10 flex flex-col gap-4 sm:flex-row">
            <a
              ...
              className="inline-flex items-center justify-center rounded-full bg-[#F4C2C2] px-8 py-4 text-base font-semibold text-[#4A2D42] shadow-lg transition hover:opacity-90"
            >

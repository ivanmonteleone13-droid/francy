...
export default function Gallery() {
  ...
  return (
    <section id="galleri" className="bg-[#4A2D42] py-20 text-white">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="mx-auto max-w-2xl text-center">
          <p className="text-sm font-semibold uppercase tracking-wider text-[#F4C2C2]">Galleri</p>
          <h2 className="mt-2 text-3xl font-bold sm:text-4xl">Fransar & naglar</h2>
        ...
        <div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {business.gallery.map((item, index) => (
            <button
              ...
              className="group overflow-hidden rounded-2xl border border-white/10 text-left transition hover:border-[#F4C2C2]/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#F4C2C2]"
            >
              {hasSingleImage(item) ? (
      ...
      {active && activeIndex !== null && hasSingleImage(active) && (
        ...
        >
          ...
          <div className="max-h-[90vh] w-full max-w-3xl overflow-hidden rounded-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="bg-[#7B4B6F] px-6 py-4 text-center text-lg font-semibold">{active.label}</p>
          </div>
        </div>
      )}
    </section>
